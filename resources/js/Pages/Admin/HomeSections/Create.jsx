...
const ContentFields = ({ sectionType, content, index, handleContentChange, multipleContent, removeContentBlock }) => {
  const [imagePreviewUrl, setImagePreviewUrl] = useState(content.image instanceof File ? URL.createObjectURL(content.image) : content.image);
  ...
  React.useEffect(() => {
    if (content.image instanceof File) {
      ...
      setImagePreviewUrl(content.image);
    }
  }, [content.image]);
...
function Create() {
  ...
  const handleSubmit = (e) => {
    ...
    onSuccess: () => toast.success("Beranda berhasil ditambahkan 🎉"),
    onError: () => toast.error("Gagal menambahkan beranda. Silakan cek kembali."),
  });
  };

  return (
    <AuthenticatedLayout header="Tambah Beranda">
    <Head title="Tambah Beranda" />
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6 font-sans">
      <div className="max-w-4xl mx-auto bg-white p-6 rounded-xl shadow-lg">
        <h1 className="text-3xl font-bold text-gray-800 mb-6 text-center">Tambah Bagian Beranda Baru</h1>
        ...
        <form onSubmit={handleSubmit} className="p-6 border border-blue-200 rounded-lg bg-blue-50">
          ...
          <button
            ...
            disabled={processing}
          >
            {processing ? 'Menyimpan...' : 'Tambahkan Bagian'}
          </button>
        </form>
      </div>
    </div>
