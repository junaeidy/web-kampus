...
export default function NewsIndex({ pages, navigations, faculties, recentNews }) {
    ...
    const displayRecentNews = recentNews && recentNews.length > 0 ? recentNews : defaultRecentNews;
                ...
                <div className="container mx-auto grid grid-cols-1 lg:grid-cols-3 gap-12">
                    <div className="lg:col-span-2"> 
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            {news.data.length > 0 ? (
                                news.data.map((item) => (
                                    <div key={item.id} className="bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300 ease-in-out hover:shadow-xl">
                                        {item.thumbnail_path && (
                                        ...
                                        <div className="p-6">
                                            ...
                                            <h3 className="text-xl font-bold text-gray-800 mb-3 leading-tight">
                                                <Link href={route('public.news.show', item.slug)} className="hover:text-custom-blue transition-colors duration-200">
                                                    {item.title}
                                                </Link>
                                            </h3>
                                            <p className="text-gray-600 text-base mb-6 line-clamp-3" dangerouslySetInnerHTML={{ __html: item.content }} />
                                ...
                                <p className="text-gray-600 text-center col-span-full">Tidak ada berita terbaru.</p>
                            )}
                        </div>
                    ...
                    <div className="lg:col-span-1 space-y-8">
                        ...
                        <div className="bg-white p-6 rounded-lg shadow-md">
                            <h4 className="text-lg font-bold text-gray-800 mb-4">Berita Terbaru</h4>
                                ...
                                {displayRecentNews.map((item) => (
                                    <Link key={item.id} href={item.url || '#'} className="flex items-center group">
                                        <div className="w-20 h-20 flex-shrink-0 mr-4 overflow-hidden rounded-lg">
                                            <img
                                                src={item.img || '/images/recent-news-default.jpg'}
                                                alt={item.title}
                                        ...
                                        <div>
                                            ...
                                            <p className="text-gray-500 text-sm flex items-center mt-1">
                                                <CalendarDays className="w-4 h-4 mr-1" /> {formatDate(item.date)}
                                            </p>
                                        </div>
                                    </Link>
                                ))}
